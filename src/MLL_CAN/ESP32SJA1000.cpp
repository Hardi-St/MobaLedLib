...
int ESP32SJA1000Class::begin(long baudRate)
{
  ...
  esp_chip_info(&chip);
  if (baudRate >= 50E3) {
//       if (chip.revision >= 2) {                                                                            // 10.07.22:  Disabled because it's overwritten below
    ...
    case (long)50E3:
      ...
      break;
        ...
        default:
          ...
          if (chip.revision >= 2) {
//             modifyRegister(REG_IER, 0x10, 0x10); // From rev2 used as "divide BRP by 2"                    // 10.07.22:  Disabled because it's overwritten below
          }else {
                return 0;
                }
      switch (baudRate) {
  ...
  else writeRegister(REG_IER, 0xff); // enable all interrupts
