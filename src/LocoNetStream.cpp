...
LocoNetClass LocoNetStream = LocoNetClass();
...
void LocoNetThrottleClass::process100msActions(void)
{
  if( myState == TH_ST_IN_USE )
  {
	...
	{
	  LocoNetStream.send( OPC_LOCO_SPD, mySlot, ( myDeferredSpeed ) ? myDeferredSpeed : mySpeed ) ;
  ...
  if( LnPacket->sd.command == OPC_SL_RD_DATA )
  {
    ...
    if( mySlot == LnPacket->sd.slot )
    {
      ...
      else if( myState == TH_ST_SLOT_MOVE )
      {
        ...
        LocoNetStream.send( OPC_LOCO_ADR, (uint8_t) ( myAddress >> 7 ), (uint8_t) ( myAddress & 0x7F ) ) ;
      }
    }
      ...
      if( myAddress == SlotAddress )
      {
        if( ( myState == TH_ST_SELECT ) || ( myState == TH_ST_DISPATCH ) )
        {
            ...
            ( LnPacket->sd.stat & LOCO_IN_USE ) != LOCO_IN_USE )
          {
            ...
            LocoNetStream.send( OPC_MOVE_SLOTS, LnPacket->sd.slot, Data2 ) ;
          }
          else
          {
            if(notifyThrottleError)
        ...
        {
          LocoNetStream.send( OPC_SLOT_STAT1, LnPacket->sd.slot, (uint8_t) ( myStatus1 & ~(STAT1_SL_BUSY|STAT1_SL_ACTIVE) ) ) ;
          mySlot = 0xFF ;
          updateState( TH_ST_FREE, 1 ) ;
        }
      }
...
TH_ERROR LocoNetThrottleClass::stealAddress(uint16_t Address)
{
  if( myState <= TH_ST_RELEASE )
  {
    ...
    LocoNetStream.send( OPC_LOCO_ADR, (uint8_t) ( Address >> 7 ), (uint8_t) ( Address & 0x7F ) ) ;
    return TH_ER_OK ;
  }
...
TH_ERROR LocoNetThrottleClass::resumeAddress(uint16_t Address, uint8_t LastSlot )
{
  if( myState <= TH_ST_RELEASE )
  {
    ...
    LocoNetStream.send( OPC_RQ_SL_DATA, LastSlot, 0 ) ;
    return TH_ER_OK ;
  }
...
TH_ERROR LocoNetThrottleClass::dispatchAddress(void)
{
  ...
  {
    updateState( TH_ST_FREE, 1 ) ;
    LocoNetStream.send( OPC_MOVE_SLOTS, mySlot, 0 ) ;
    return TH_ER_OK ;
  }
...
TH_ERROR LocoNetThrottleClass::acquireAddress(void)
{
  if( myState <= TH_ST_RELEASE )
  {
    ...
    LocoNetStream.send( OPC_MOVE_SLOTS, 0, 0 ) ;
    return TH_ER_OK ;
  }
...
TH_ERROR LocoNetThrottleClass::releaseAddress(void)
{
  if( myState == TH_ST_IN_USE )
  {
    LocoNetStream.send( OPC_SLOT_STAT1, mySlot, (uint8_t) ( myStatus1 & ~(STAT1_SL_BUSY) ) ) ;
...
TH_ERROR LocoNetThrottleClass::idleAddress(void)
{
  if( myState == TH_ST_IN_USE )
  {
    LocoNetStream.send( OPC_SLOT_STAT1, mySlot, (uint8_t) ( myStatus1 & ~(STAT1_SL_ACTIVE) ) ) ;
...
TH_ERROR LocoNetThrottleClass::freeAddress(void)
{
  if( myState == TH_ST_IN_USE )
  {
    LocoNetStream.send( OPC_SLOT_STAT1, mySlot, (uint8_t) ( myStatus1 & ~(STAT1_SL_BUSY|STAT1_SL_ACTIVE) ) ) ;
...
TH_ERROR LocoNetThrottleClass::setSpeed(uint8_t Speed )
{
  if( myState == TH_ST_IN_USE )
  {
    ...
    if( mySpeed != Speed )
    {
      ...
      {
        LocoNetStream.send( OPC_LOCO_SPD, mySlot, Speed ) ;
        myTicksSinceLastAction = 0 ;
        myDeferredSpeed = 0 ;
      }
    }
...
TH_ERROR LocoNetThrottleClass::setDirection(uint8_t Direction )
{
  if( myState == TH_ST_IN_USE )
  {
    LocoNetStream.send( OPC_LOCO_DIRF, mySlot,
...
TH_ERROR LocoNetThrottleClass::setFunction(uint8_t Function, uint8_t Value )
{
  ...
  if( myState == TH_ST_IN_USE )
  {
    ...
    LocoNetStream.send( OpCode, mySlot, Data ) ;
...
TH_ERROR LocoNetThrottleClass::setDirFunc0to4Direct(uint8_t Value )
{
  if( myState == TH_ST_IN_USE )
  {
    LocoNetStream.send( OPC_LOCO_DIRF, mySlot, Value & 0x7F ) ;
    return TH_ER_OK ;
  }
...
TH_ERROR LocoNetThrottleClass::setFunc5to8Direct(uint8_t Value )
{
  if( myState == TH_ST_IN_USE )
  {
    LocoNetStream.send( OPC_LOCO_SND, mySlot, Value & 0x7F ) ;
    return TH_ER_OK ;
  }
...
TH_ERROR LocoNetThrottleClass::setSpeedSteps(TH_SPEED_STEPS newSpeedSteps)
{
  ...
  if((myState == TH_ST_IN_USE) && ((myStatus1 & 0x07) != mySpeedSteps))
  {
  	...
  	LocoNetStream.send(OPC_SLOT_STAT1, mySlot, myStatus1);
  }
  updateSpeedSteps(mySpeedSteps, 1);
	return TH_ER_OK;
}
...
{
  LocoNetStream.send( OPC_RQ_SL_DATA, FC_SLOT, 0 ) ;
}
    ...
    if( fcSlotData.frac_minsl & 0x80 )
    {
      ...
      if( fcSlotData.frac_minsh & 0x80 )
      {
		...
		if( fcFlags & FC_FLAG_MINUTE_ROLLOVER_SYNC )
		{
			...
			LocoNetStream.send((lnMsg*)&fcSlotData) ;
		}
		else
			doNotify( 0 ) ;
      }
...
bool LocoNetSystemVariableClass::CheckAddressRange(uint16_t startAddress, uint8_t Count)
{
  ...
  while (Count != 0)
  {
    ...
    {
       LocoNetStream.sendLongAck(42); // report invalid SV address error
       return 0;
    }
    startAddress++;
    Count--;
...
#ifdef NO_SOFTWARE_UART	
void (* ResetProcess)(void) = 0; // Restart the process
#endif	
...
SV_STATUS LocoNetSystemVariableClass::processMessage(lnMsg *LnPacket )
{
  ...
  switch( LnPacket->sv.sv_cmd )
  {
    ...
    case SV_CHANGE_ADDRESS:
        ...
        {
          LocoNetStream.sendLongAck(44);  // failed to change address in non-volatile memory (not implemented or failed to write)
    ...
    default:
        LocoNetStream.sendLongAck(43); // not yet implemented
        return SV_ERROR;
  }
  ...
  LN_STATUS lnStatus = LocoNetStream.send(LnPacket, LN_BACKOFF_INITIAL);
  ...
  if (lnStatus != LN_DONE) {
    ...
    LocoNetStream.sendLongAck(44);  // indicate failure to send the reply
  }
				...
				if (LnPacket->ub.payload.data.deviceClass == 0xFFFF && LnPacket->ub.payload.data.lncvNumber == 0x0000 && LnPacket->ub.payload.data.lncvValue == 0xFFFF) {
					...
					if (notifyLNCVread) {
						...
						if (returnCode == LNCV_LACK_OK) {
							...
							LocoNetStream.send(OPC_LONG_ACK, old_opcode, returnCode);
							// return a nack
							ConsumedFlag = 1;
						}
					}
					...
					if ((LnPacket->ub.payload.data.flags & LNCV_FLAG_PRON) != 0x00 && ((LnPacket->ub.payload.data.flags & LNCV_FLAG_PROFF) != 0x00)) {
						...
						if (notifyLNCVprogrammingStart) {
							...
							if (notifyLNCVprogrammingStart(LnPacket->ub.payload.data.deviceClass, LnPacket->ub.payload.data.lncvValue) == LNCV_LACK_OK) {
								...
								#endif
								LocoNetStream.send((lnMsg*)&response);	
								#ifdef DEBUG_OUTPUT
								LN_STATUS status = LocoNetStream.send((lnMsg*)&response);	
