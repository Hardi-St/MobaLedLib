/*
 ...
 - Excel aktualisieren
   Die vielen Seiten brauchen sehr viel Platz. Evtl. kann man nur die Daten der Seiten in einer
   einzigen versteckten Seite speichern und die Seiten dann daraus generieren.
 - README.md erweitern => O.K.
 - keywords.txt aktualisieren

 - Mail an skrauss wenn es endlich fertig ist

 - S3PO Modul Ausgaenge testen / Platine machen fuer die Strassenlampen mit Gluehbirne.
 - Ifdef mit dem man das TV Flackern lokal berechnen kann. Damit spart man sich 10 Byte RAM                   => Nein das bleibt so
 - Zufaellige und periodische wiedergabe von Sounds (Tiere / Kirchturm / Oder Bahnhofsansage)
   Hier kann auch die Logik Funktion eingesetzt werden ( Taste oder Automatik)
 - Man koennte eine Helligkeitsanpassung machen indem man die Farbe bei dem LEDs auf 1 setzt welche
   Automatisch an die Helligkeit angepasst werden sollen. Nach dem MobaLedLib.Update() Aufruf
   koennten diese LEDs dann auf einen Wert gesetzt werden der zu der Umgebungshelligkeit passt.
   Das muss aber ausserhalb der Lib gemacht werden.
   Die FastLED.setBrightness(MAX_BRIGHTNESS) Funktion bezieht sich auf alle LEDs.
 - Push Button Action einlesen
   - Video / Fotos machen

 - InpStructArray[] beschreiben
   - Aufbau
   - Wann welche Bits gesetzt / Geloescht werden
   - Problematik beim Setzen im Interrupt
   - Funktionen welche das InpStructArray benutzen
     - evtl. diese in eigenes Modul auslagern
 - Zufallssteuerung soll auch eine Global_Var nutzen
   - Schedule ?

 - Untersuchen wie viel es bringt, wenn man die IncCP_... nicht nach der Proc_... switch() einzeln aufruft sondern
   die entsprechende Funktion
   - Anstelle des switch/case koennte man auch eine Tabelle mit funktionspointern benutzen. Das sollte weniger
     Speicher belegen und schneller sein.

 - Ansteuerung der Conrad „Servos“
   - Peak zu Beginn, dann PWM langsam steigern.
   - Mit zusaetzlicher Negativer Versorgung
   - Verriegelung damit nicht beide Transistoren gleichzeitig an sind
 - Man koennte auch Weichen mit den WS2811 Modulen anteuern.
   Hier braeuchte man aber eine zusaetzliche Sicherheitsabschaltung
   und viele Leistungstransistoren. Ein Positionsrueckmeldung ist nicht moeglich.                               => Das bringt nichts

 - Eigentlich muesste das Feuer auch langsam ausgehen wie die Gaslichter. Nur viel langsamer...
   Das widerspricht aber der Idee, dass immer ein Licht sofort aus geht wenn abgeschaltet wird.
 - _USE_SEP_CONST auf 0/1 umstellen. Dazu muss aber die Create_lib.bat angepasst werden
 - Signalbilder: https://wiki.rocrail.net/doku.php?id=signaling-de
 - Lesen:
   - https://github.com/arduino/arduino/wiki/arduino-ide-1.5:-library-specification
   - https://www.arduino.cc/en/Reference/APIStyleGuide
   - https://www.arduino.cc/en/Reference/StyleGuide
 - Es waehre schoen, wenn man wichtige Optionen der Library ohne Aenderungen an Lib_Config.h Ein- und
   Ausschalten koennte.
   - Folgende Optionen sind NICHT in der Bibliothek:
     - USE_LDR
     - CAN_INPUT
 ...
 - Dekoder Funktion mit der mehrere Eingaenge einen WS2811 Ausgang ueber eine Tabelle steuern kann.
   Das braucht man fuer die neue Sound funktion (wenn das so geht) und fuer die Steuerung mehrerer
   Servos ueber einen ATTiny und einen oder mehrere Analog Eingaenge.
   Die Servos koennen auch sequenziel gesteuert werden 10 verschiedene analoge Spannungen kann man
   problemlos setectieren. Damit koennte man 5 Servos steuern. Eine Spannung fuer "fahre nach rechts",
   eine fuer "fahre nach links". Ein ATTiny85 hat 5 Pins => Ein Analog Eingang, 4 Servos.
   Am ATTiny84 koennte man sogar 8 Servos anschliessen.
 - Bescheibung zu den Beispielen in "Pattern_Configurator.xlsm"
 - Blaulicht Dauer messen und dokumentieren
 - Sourcen kommentieren / Aufraeumen
 - C++ Interface anstelle des Configuration Arrays ?

 - Temporaere Variblen 251 - 253 fuer zwischenwerte der Logic Funktion => Nein, keine speziellen Temp Vaiablen
 - Die Pattern Funktion koennte auch zufaellige Zeiten bekommen.
   Dazu koennte man ganz grosse Zeiten fuer Zufallssteuerung verwenden.
   Zeiten ab 65000 definieren Zufaellige Zeiten.
   21 Werte fuer MinTime 0.5 sek - 10 Sek, 21 Werte fuer die MaxTime
   Die Zeiten Koenten ueber eine Quadratische Funktion berechnet werden
   damit man aus 21 verschiedenen Werten ine grosse Spreitzung bekommt.
 - Andere Spezial Funktionen (FlipFlop, ...) welche die Variablen VAR() benutzen.
 - Haus
   - Disco Licht. Hier evtl. mehrere RGB LED zusammenfassen damit das Licht aus unterschiedlichen
     Richtungen kommt. Das kann man mit der RandMux() Funktion machen
 - Die mod8() Funktion ist in zwei Faellen langsamer als der "%" Opreator => Die anderen Stellen untersuchen
 - Konstanten welche in der Leds_Config verwendet werden duerfen sollen in eine eigne Datei.
   Sollen da auch die Makros rein ?
   Interne Konstanten koennte man auch mit einem vorangestellten '_' markieren
 - Macro zum auswerten einer bestimten nummerischen Bitkombination. Das koennte man zur Auswahl der
   Signalbilder benutzen. Bits2Number3(Var, FirstInp, MatchNr)
 - Erkennung ob LDR oder Taster angeschlossen ist. Evtl. auch automatische CAN BUS Erkennung.
 - Weitere Ueberprufungen in die Markros einbauen und im Fehlerfall eine Fehlernummer X*256 generieren (Siehe ERR_INPUT_RANGE)
 - Excel:
   - Automtische korrektur der Zeiten und setzen des PF_SLOW Flags wenn lange Zeiten verwendet werden.
   - Anzeige der moeglichen Modies
   - Ungueltige Werte verhindern
     Wenn man einen falschen Startkanal in dem Excel Sheet angibt (z.B.9, dann stimmt nichts mehr)
     => Nur gueltige Eingaben erlauben
     ==> Problem aufgetreten wegen altem Excel Blatt ;-(
   - Beispiel und Texte aktualisieren
   - Diagramm im Analog Mode
   - Balkendiagram im Digitalmode
 - Powermanagement Funktionen von FastLED Dokumentieren
 - Untersuchen ob man fuer die Const() Funktion ohne Zeiten und ohne Pattern auskommen koennte
   => Nein nicht so einfach
 - Bei den Tests hat ploetzlich das Feuerwehrauto und die Fade1..3 LEDs wild geflackert. Nach dem ich das       // 27.08.18:
   Programm neu geschickt hab war wieder alle o.K. Weiss nicht welche Tastenkombination das ausgeloest hat
 - Programm aufraeumen
   - #defines pruefen und #error Ausgeben
   - Dokumentieren
 - Soll es ein PatternT0() Makro geben fuer die Const() Funktion?
   Das wuerde zwei Byte in der Konfiguration sparen, allerdings evtl. zusaetzliche if abfragen ...
   => untersuchen
 - Die FastLED Bibliothek kann mehrere Stripes parallel ansteuern. Dadurch reduziert sich die
   Updatezeit entsprechend !! as kann u.U. sinnvoll sein weil dadurch die Interrupts kuerzer gesperrt werden.
 - Die Anzahl der LEDs soll erkannt werden (Lesen von Pin 5) und im Testmode zur limmitierung der LedNr
   verwendet werden. Problem: Waehrend des Updates der LEDs in der FastLED Bibliothek sind die Interrupts
   abgeschaltet. => Die Funktion muesste in die Bibliothek eingebaut werden.
 - CAN Timeout: Zu Debug Zwecken kann es sinnvoll sein, wenn nach dem Anschalten ohne CAN Bus alle LED's
   nach 2 Sekunden an gehen.
 - Servo fuer Signale (Signal mit ATTiny einlesen und umwandeln)
 - Trigger der Sound Module
 - Performance Tests
   - Refreshrate
     => Frames per Second:   100 LEDs  218 Fps: 4 ms
                              50 LEDs  330 Fps: 3 ms
                              25 LEDs  400 Fps: 2.5 ms
                            => 1 LED ~ 20 us
   - Speicher Verbrauch bei 100 LED's  => Momentan 30%
   - Untersuchen wie lange die pgm_read_byte_near() Funktion braucht im vergleich zum lesen aus dem RAM
     => pgm_read_byte_near() ist gleich schnell wie RAM zugriff !!


 - Analog Inputs: Verwendet Interrupts, und hat Callback Funktion: https://github.com/merose/AnalogScanner
     ...
     *Last_t = t & 0xFF;
     ...
     if (Inp_Is_On(Inp))
          ...
          for(uint8_t k= LED_cnt - 3; k > 0; k--)
            heat[k] = (heat[k + 1] + heat[k + 2] + heat[k + 2] ) / 3;   // 15.04.18:  Old - 1, -2

          // Step 3.  Randomly ignite new 'sparks' of heat near the bottom
          if (random8() < SPARKING )
             {
             uint8_t y = random8(min(7,LED_cnt));
...
#                                                                                                             endif
#                                                                                                             if _USE_INCH_TO_VAR
      case INCH_TO_TMPVAR_T:IncCP_InCh_to_TmpVar(); break;
#                                                                                                             endif
#                                                                                                             if _USE_LOGIC
