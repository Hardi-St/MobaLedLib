...
#define SWITCH0_PIN 7          // Left   push button on the mainboard
#define SWITCH1_PIN 8          // Middle push button on the mainboard
#define SWITCH2_PIN 9          // Right  push button on the mainboard
...
#if defined TEST_BUTTONS_INCH && !defined TEST_TOGGLE_BUTTONS  && !defined TEST_PUSH_BUTTONS                  // 14.01.20:
  #define TEST_TOGGLE_BUTTONS
#endif

#if defined TEST_TOGGLE_BUTTONS || defined TEST_PUSH_BUTTONS                                                  // 06.10.19:

   #ifndef TEST_BUTTONS_INCH
     #define TEST_BUTTONS_INCH 0
   #endif
   ...
   //----------------------------
   void Check_Mainboard_Buttons()
   //----------------------------
   // Use the 3 buttons of the main board to change input channels
   // 12.01.20:  - Improved the function
   //              - new define TEST_PUSH_BUTTONS
   //              - detect changes from DCC and update the LED
   //              - corrected the LastKey check. Now the InpCh are only set in case the buttons are pressed
   {
     {
     ...
     for (uint8_t KeyNr = SWITCH0_PIN, LEDNr = LED1_PIN, InChNr = TEST_BUTTONS_INCH; KeyNr <= SWITCH2_PIN; KeyNr++, LEDNr++, InChNr++)
         {
         // Check if the input was changed by DCC, ...
         uint8_t Inp = MobaLedLib.Get_Input(InChNr);
         if ((LastKey & FIRST_FLAG) || Inp_Changed(Inp))
            {
            //Dprintf("First %02X\n", Inp); // Debug
            digitalWrite(LEDNr, Inp_Is_On(Inp));
            }

         // Periodic check if a button is pressed / released
         if (Mask) // Mask is used in addition as Flag for the periodic button check
            {
            uint8_t Key = !digitalRead(KeyNr);
            if (Key != ((LastKey & Mask)?1:0))
               {
               #ifdef TEST_PUSH_BUTTONS                                                                       // 08.01.20:
                  if (Key)
                       LastKey |=  Mask;
                  else LastKey &= ~Mask;
                  digitalWrite(LEDNr, Key);
               #else // TEST_TOGGLE_BUTTONS
                  if (Key)
                       { LastKey |=  Mask; digitalWrite(LEDNr, !digitalRead(LEDNr)); }
                  else { LastKey &= ~Mask; }
                  NextCheck = millis() + 100;                                                                  // 08.01.20:  Old 300
               #endif
               MobaLedLib.Set_Input(InChNr, digitalRead(LEDNr));
               //Dprintf("%02X %02X %02X\n", LastKey, Mask, Key); // Debug
               }
            Mask <<= 1;
            }
         }
     LastKey &= ~FIRST_FLAG; // Clear the first flag
     }
   }
#endif // TEST_TOGGLE_BUTTONS || TEST_PUSH_BUTTONS
...
//-----------------------
{
    ...
    FAST_SET_MAINBOARDLED(_D2,  D, B00010000); // D4  = PortD4
  #endif
  ...
  // Wenn ein InCh angegeben ist, dann startet wechselt der Zähler bei
  ...
  // Normalerweise wird der Darkness Zähler bei einer Flanke auf den
  ...
  // Wenn KeepDarknessCtr definiert ist, dann wird der Darkness Zähler
  // bei einer Flanke nicht auf den entsprechenden Startwert gesetzt.
  // Dadurch Ergibt sich kein Spring im Zählerstand.
