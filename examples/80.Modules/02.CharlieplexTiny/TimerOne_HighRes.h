 ...
 *
 *  Hardi:
 *  - changed the microseconds parameter to double to be able to set a more acurate period
 *
 */
...
class TimerOne
{
  ...
  public:
    ...
    //  Configuration
    //****************************
    void initialize(double microseconds=1000000) __attribute__((always_inline)) {
	TCCR1B = _BV(WGM13);        // set mode as phase and frequency correct pwm, stop the timer
	TCCR1A = 0;                 // clear control register A
	setPeriod(microseconds);
    }
    void setPeriod(double microseconds) __attribute__((always_inline)) {
    ...
    void pwm(char pin, unsigned int duty, double microseconds) __attribute__((always_inline)) {
    ...
    void attachInterrupt(void (*isr)(), double microseconds) __attribute__((always_inline)) {
	if(microseconds > 0) setPeriod(microseconds);
	attachInterrupt(isr);
    }
